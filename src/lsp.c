...
static const struct qstar_lsp_hover_entry qstar_lsp_symbols[] = {
	...
	{ "qstar.objectlib", "Create a generic object collection target consumed through objects = { ... }; compile_context is own or consumer." },
	{ "qstar.interface", "Declare an artifact-free dependency target with explicit compile_usage and link_usage requirements." },
	...
	{ "qstar.stage", "Create a copy-only package staging rule." },
	...
	{ "qstar.step.check", "Project command step that runs graph/input validation." },
	{ "qstar.step.lint", "Project command step that runs authoring lint." },
	{ "qstar.step.run", "Project command step that executes a captured command-local argv-vector action with inputs, env, timeout, expect, action-log, and replay." },
	{ "qstar.step.call", "Project command step that invokes another project command." },
	{ "qstar.step.export_stage", "Project command step that materializes a stage and copies the layout to a package-relative destination." },
	{ "qstar.param", "Reference a project command runtime option inside qstar.cli or step conditions." },
	{ "qstar.param.string", "Project command option schema for a string-valued CLI option." },
	{ "qstar.param.path", "Project command option schema for a package-relative path option." },
	{ "qstar.param.bool", "Project command option schema for a boolean CLI flag." },
	{ "qstar.param.int", "Project command option schema for an integer CLI option." },
	{ "qstar.param.enum", "Project command option schema constrained to declared choices." },
	{ "qstar.param.list", "Project command option schema for a repeated string option." },
	{ "qstar.arg_if", "Append one project command argv item when a bool command option is true." },
	{ "qstar.args_if", "Append multiple project command argv items when a bool command option is true." },
	...
	{ "qstar.tool_file", "Reference the executable path of qstar.tool, imported role=tool, executable, or test target." },
...
static const struct qstar_lsp_hover_entry qstar_lsp_fields[] = {
	...
	{ "objects", "Object library labels consumed by an artifact target." },
	...
	{ "implementation", "Provider implementation Lua file loaded in the restricted provider sandbox." },
	{ "options", "Provider-defined language option schema or helper-exported language option table." },
	{ "scaffold", "Optional provider init scaffold metadata validated as declarative data." },
	{ "shapes", "Provider scaffold map for app, lib, tool, empty, and workspace project shapes." },
	{ "files", "Provider scaffold file declarations with package-relative paths and literal bodies." },
	{ "fragments", "Provider scaffold workspace fragment declarations using package-relative .qst paths." },
	{ "type", "Project option or provider option type." },
	...
	{ "compile_usage", "Dependency usage requirements with explicit options and rebuild inputs applied to consumers." },
	{ "link_usage", "Dependency link usage requirements with explicit options and rebuild inputs applied to consumers." },
	{ "artifact_kind", "Descriptive imported target kind metadata; it never infers compiler or linker flags." },
	{ "artifacts", "Platform-keyed imported artifact lists with id, role, path, and primary metadata." },
	{ "role", "Imported artifact role such as link, runtime, tool, or a project-defined descriptive identifier." },
	{ "primary", "Selects the default target_file artifact for one imported platform artifact list." },
	{ "public_headers", "Language-local exported headers under lang.c or lang.cxx." },
	{ "private_headers", "Language-local private headers under lang.c or lang.cxx." },
	...
	{ "precompiled_header", "Optional package-relative C++ precompiled header under lang.cxx." },
	{ "unity", "Opt-in C++ unity compilation table with enabled and batch_size." },
	{ "batch_size", "Number of C++ implementation sources per unity translation unit." },
	{ "modules", "Opt-in C++ module interface and implementation lowering under lang.cxx." },
	{ "enabled", "Boolean switch used by opt-in language strategy tables." },
	{ "command", "argv-vector command built with qstar.cli." },
	{ "description", "Validated progress/status message built with qstar.status." },
	{ "timeout", "run_target or qstar.step.run timeout in seconds." },
	{ "expect", "run_target or qstar.step.run expectation table with contains and optional file." },
	{ "contains", "Expected text for run_target or qstar.step.run checks under expect.contains." },
	{ "file", "Optional package-relative file scanned by run_target or qstar.step.run expect.file." },
	{ "root", "Stage root path for qstar.stage, package-relative." },
	{ "files", "Stage file mappings built with qstar.stage_file." },
	{ "steps", "Ordered qstar.step.* list inside a root qstar.command." },
	{ "is_default", "Marks the default root project command; only one command may set it." },
	{ "hidden", "Hides a project command from qstar commands while keeping it callable." },
	{ "aliases", "Alternative CLI names for a root project command." },
	{ "working_dir", "Package-relative default working directory for command run steps." },
	{ "to", "Package-relative export_stage destination, optionally supplied by qstar.param(\"path_option\")." },
	{ "variants", "Target family variant names used only for lint grouping." },
	...
	{ "artifact_name", "Target artifact filename override, for example app-tool." },
	{ "libs", "System libraries rendered by the selected link policy." },
