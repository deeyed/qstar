...
qstar --file qstar.lua -B out/qstar -G stella build //:app
...
- full Ninja parity for `sharedlib` and Cale source actions
...
- CLI `-G stella|ninja|auto` 또는 `--generator`는 effective generator를 선택한다.
  기본 action 실행 backend는 `stella`이며, `auto`도 `stella`로 resolve된다.
  ...
  `build/qstar/ninja/build.ninja`로 lower해 실행한다. `stage`와 `install`은 copy와
  manifest는 QStar가 처리하되, 참조 artifact build에는 effective generator를 사용한다.
...
`//pkg:target`/generated action label을 검색한다. Round 81부터
`qstar.import_file("path.qst")`와 `qstar.import_module("folder/path")` 문자열에서
definition은 각각 `.qst` fragment와 `<name>/<name>.qsm` helper module로 이동한다.
`workspace/didChangeWatchedFiles`는
다음 라운드 후보로 남긴다.
...
`[5%] Stage 1: ...` 형태의 compact progress와 최종 `status ok/fail` summary만
출력한다. `--verbose`는 tick 사이에 `Status: compiling ...` 같은 상태 line과
`build_action` 세부 event를 추가하고,
...
Experimental surface는 `cale build` integration, remote package resolver, 남은 Ninja
sharedlib/Cale source parity, full sharedlib executor, HCL semantics, Cale internal API integration으로
분리한다.
...
남은 Ninja sharedlib/Cale source parity, full sharedlib executor, Cale internal compiler API, C++ modules,
