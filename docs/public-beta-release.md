...
```txt
runtime version: qstar 0.7.2-beta
release tag: v0.7.2-beta
macOS asset: qstar-v0.7.2-beta-macos-arm64.tar.gz
Linux asset: qstar-v0.7.2-beta-linux-x86_64.tar.gz
Windows planned asset: qstar-v0.7.2-beta-windows-x86_64.zip
...
현재 patch release line은 `0.7.2-beta`다. `v0.7.0-beta` tag는 이전 feature beta
release로, `v0.7.1-beta` tag는 이전 patch release로 보존하고 이동하지 않는다.
Default `qstar build` path는 바꾸지 않는다.
Release note는 `docs/releases/v0.7.2-beta.md`, 판단 기준은
...
test -f dist/release/qstar-v0.7.2-beta-linux-x86_64.tar.gz
...
`workflow_dispatch` input `publish_linux_asset=true`와 `release_tag=v0.7.2-beta`를 지정해
...
test -f dist/release/qstar-v0.7.2-beta-windows-x86_64.package-plan.txt
test -f dist/release/qstar-v0.7.2-beta-windows-x86_64.expected-contents.txt
```
...
```sh
tar -xzf qstar-v0.7.2-beta-macos-arm64.tar.gz -C "$HOME/.local"
export PATH="$HOME/.local/bin:$PATH"
qstar --version

tar -xzf qstar-v0.7.2-beta-linux-x86_64.tar.gz -C "$HOME/.local"
export PATH="$HOME/.local/bin:$PATH"
qstar --version
```
...
```sh
unzip -q qstar-v0.7.2-beta-windows-x86_64.zip -d "$HOME/.local"
export PATH="$HOME/.local/bin:$PATH"
qstar.exe --version
```
...
```sh
git tag -a v0.7.2-beta -m "QStar v0.7.2 beta"
git push origin v0.7.2-beta

gh release create v0.7.2-beta \
  dist/release/qstar-v0.7.2-beta-macos-arm64.tar.gz \
  dist/release/SHA256SUMS \
  --repo deeyed/qstar \
  --title "QStar v0.7.2 Beta" \
  --notes-file docs/releases/v0.7.2-beta.md \
  --prerelease \
  --latest=false
```
