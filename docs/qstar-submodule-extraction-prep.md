...
기록한다. downstream repository는 `qstar/`를 이 repository의 submodule pin으로 소비한다.
...
`docs/`는 QStar standalone repository의 문서 root로 취급한다. downstream repository의
`docs/qstar/`는 상세 mirror가 아니라 이 submodule 문서를 가리키는 얇은 handoff만
유지한다.

## external language Side Conversion Contract

downstream repository에서 QStar를 소비할 때는 다음 경계를 유지한다.

- external language `.gitmodules`: `qstar/` entry만 둔다.
- QStar standalone `.gitmodules`: `vendor/lua` entry를 소유한다.
- downstream repository: `qstar/` directory를 QStar remote submodule로 둔다.
- downstream root docs: QStar 상세 문서 대신 submodule 문서를 가리키는 얇은 handoff만 남김.
- Local install: standalone repo 기준 `make install PREFIX=$HOME/.local` 또는 external language
  submodule 기준 `make -C qstar install PREFIX=$HOME/.local`로 `~/.local/bin/qstar`를
  갱신한다.
- VSCode extension: standalone repo 또는 external language submodule의 `editors/vscode/qstar`에서
...
downstream repository는 QStar implementation file을 mirror하지 않는다. downstream build integration이
필요해지면 external language side glue만 downstream repository에서 관리한다.
