...
```txt
qstar/build/bin/qstar
  -> .qstar/state/actions.json action manifest
...
qstar/build/bin/qstar
  ...
  -> .qstar/stage/<label>/manifest.json and dry-run diff
```
...
- test stdout/stderr are stored in `.qstar/logs`.
...
- stdout, stderr, and action logs are stored under `.qstar/logs`.
- build artifacts are stored under `.qstar/out`.
...
- QStar is built and checked from `qstar/`; the root Cale `Makefile` does not
  own QStar targets.
- `.qstar/state/actions.json` records action id, key, output, and last status.
...
- `qstar clean` removes `.qstar` and `compile_commands.json`; `qstar clean
...
- stdout/stderr/action logs stay under `.qstar/logs`.
- `.qstar/logs/last-failure.replay` includes the package-root cwd and argv.
