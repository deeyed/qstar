...
- `perf/q166-large-performance-refresh.md`: 200/500 target large synthetic Stella/Ninja/daemon repeat-3 timing refresh.
...
- `windows-artifact-graph-ir.md`: Q173 Windows `.exe`/static `.lib`/runtime `.dll`/import `.lib` Graph IR and multi-output implementation plan.
...
Round Q157 release cleanup부터 runtime line은 `0.6.1-beta`로 이동한다. Release note는
