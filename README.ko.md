...
바이너리만 먼저 배포한다. Linux는 Ubuntu gcc/clang CI 기반 source build 검증 경로를
갖췄고 Windows는 검증 예정이다. QStar 1.0은 macOS, Linux, Windows 공식 지원이 모두
갖춰진 뒤에 올린다.
...
| Linux | Ubuntu gcc/clang CI 기반 source build 검증 |
| Windows | 검증 예정 |
