...
MSYS2 UCRT64 기반 validation-backed beta candidate 단계이고 `windows-x86_64` zip package
skeleton을 갖지만, 아직 배포된 public asset이나 official host support는 없다.
...
qstar test //...
qstar commands
qstar install --out exports/install  # qstar.lua가 선언한 project command
...
| Windows | MSYS2 UCRT64 기반 validation-backed beta candidate, zip package skeleton, 배포된 public asset 없음 |
...
sharedlib runtime/import artifact와 release package dry-run plan을 검증하지만 Windows release
asset은 아직 배포하지 않는다.
