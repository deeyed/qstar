...
- Configurable build surface current public reference는
  `docs/configurable-build-surface.md`와
  `wiki/reference/configurable-build-surface.md`다. 이 reference는 `qstar.option`,
...
- Windows host 지원은 아직 official support가 아니다. Q225 기준 Windows는
  ...
  sharedlib parity, install docs/man smoke, Windows release asset smoke를 실행하고
  `qstar-windows-beta-candidate`
  ...
  `windows-sharedlib-detail/`, `release-package/`, `release-asset-smoke/`을 포함해 실패
  class와 future package layout/smoke를 raw log만으로 추적하지 않게 한다. 별도
  `qstar-windows-beta-release-asset` artifact는 zip candidate, `SHA256SUMS`, package reports,
  extracted smoke logs를 담는다.
  ...
  release asset evidence는 "보유" 상태다. 다음 beta/RC tag와 v1 candidate tag에서는
  같은 `publish_windows_asset=true` release-mutating gate를 반복해야 하며, 단순
  `--ref main` freshness run은 이 반복 evidence를 대체하지 않는다. 상세 정책은
...
`steps`에는 `qstar.step.build/test/stage/check/lint/run/call/export_stage`가 들어가며,
`qstar commands`로 목록을 확인한다. Command option은 `qstar.param.*`로 선언하고, runtime argv에서는
`qstar.param("name")`, `qstar.arg_if`, `qstar.args_if`를 쓴다. `when`과 conditional argv
helper의 condition은 bool option만 참조할 수 있다. `qstar.step.run`은 `inputs`,
`timeout`, `expect`, `env`를 지원하고 stdout/stderr/action-log/replay를 `qstar.run_target`
