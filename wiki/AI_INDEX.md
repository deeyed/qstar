...
- Persistent Stella daemon은 Q147 기준 memory-state streaming experimental MVP다. 명령 namespace는
  ...
  복구 가능하다. Authoring input mtime/size가 바뀌면 graph를 다시 load한다. 일반 source/header
  변경은 기존 Stella dirty-check가 처리한다.
  ...
  `deps_memory status=hit|miss|writeback`으로 memory path를 확인한다. File watcher,
  background start/stop, Windows named pipe는 deferred다. 정본 설계는 `docs/daemon/stella-daemon.md`,
