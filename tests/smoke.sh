...
contains "$tmp/ui-compact.out" "[100%] Stage"
...
test "$(cat "$tmp/version-flag.out")" = "qstar 0.4.0-beta.1" || fail "qstar --version drifted"
"$qstar" version > "$tmp/version-cmd.out" 2> "$tmp/version-cmd.err"
test "$(cat "$tmp/version-cmd.out")" = "qstar 0.4.0-beta.1" || fail "qstar version drifted"
...
contains "$tmp/lua-authoring.out" "-DQSTAR_VERSION=0.4.0-beta.1"
contains "$tmp/lua-authoring.out" "-DQSTAR_VERSION_MINOR=4"
...
contains "docs/public-beta-release.md" "qstar-v0.4.0-beta.1-macos-arm64.tar.gz"
