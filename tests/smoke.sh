...
QSTAR_DEBUG_STATE_DUMPS=1 "$qstar" --file "$tmp/qstar.lua" build //:app --progress off > "$tmp/debug-state.out" 2> "$tmp/debug-state.err"
test -f "$tmp/build/qstar/state/actions.json" || fail "missing opt-in debug action state"
...
rm -f "$tmp/build/qstar/state/actions.json"
